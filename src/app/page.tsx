  ...
  useEffect(() => {
    async function fetchImpactData() {
      try {
        ...
        setImpactData({
          wasteCollected: Math.round(wasteCollected * 100) / 100,
          reportsSubmitted,
    ...
    <div className={`container mx-auto px-4 py-16 ${poppins.className}`}>
      <section className="text-center mb-20">
        ...
        <h1 className="text-6xl font-bold mb-6 text-foreground tracking-tight">
          EcoRise <span className="text-green-600">Waste Management</span>
        </h1>
      ...
      <section className="bg-card p-10 rounded-3xl shadow-lg border border-border mb-20">
...
function ImpactCard({ title, value, icon: Icon }: { title: string; value: string | number; icon: React.ElementType }) {
  ...
  return (
    <div className="group relative p-6 rounded-xl bg-card border border-border transition-all duration-300 ease-in-out hover:shadow-[0_0_20px_rgba(34,197,94,0.4)] hover:-translate-y-1 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-r from-red-500 via-yellow-500 via-green-500 via-blue-500 to-purple-500 opacity-0 group-hover:opacity-10 transition-opacity duration-300"></div>
      <Icon className="h-10 w-10 text-green-500 mb-4 group-hover:text-green-600 transition-colors" />
      <p className="text-3xl font-bold mb-2 text-foreground">{formattedValue}</p>
...
function FeatureCard({ icon: Icon, title, description }: { icon: React.ElementType; title: string; description: string }) {
  return (
    <div className="group relative bg-card p-8 rounded-xl border border-border shadow-md transition-all duration-300 ease-in-out flex flex-col items-center text-center hover:shadow-[0_0_25px_rgba(0,0,0,0.2)] dark:hover:shadow-[0_0_25px_rgba(255,255,255,0.1)] hover:-translate-y-2 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-red-500 via-yellow-500 via-green-500 via-blue-500 to-purple-500 opacity-0 group-hover:opacity-10 transition-opacity duration-300"></div>
      <div className="bg-green-100 dark:bg-green-900/30 p-4 rounded-full mb-6 transition-transform group-hover:scale-110">
